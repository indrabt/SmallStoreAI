...
class PerishableInventoryTracker:
    ...
    def _ensure_data_file_exists(self):
        ...
        if not os.path.exists(self.data_file):
            ...
            sample_data = {
                f"inventory_items": self._generate_sample_inventory(),"
                ...
                ff  f"notification_emails": ["manager@smallstore.com"],"
...
try:
                ...
                json.dump(sample_data, file, indenft=4)'
    
    def _generate_sample_inventory(self):
f       f"""Generate sample perishable inventory for demonstration""""
    ...
    f       {f"name": "Milk", "category": "Dairy", "shelf_life_days": 10, "current_stock": 45},
            {"name": "Yogurt", "category": "Dairy", "shelf_life_days": 30, "current_stock": 65},"
    f        {f"name": "Bread", "category": "Bakery", "shelf_life_days": 7, "current_stock": 30},
            {"name": "Apples", "category": "Produce", "shelf_life_days": 14, "current_stock": 120},"
f         {f"name": "Bananas", "category": "Produce", "shelf_life_days": 7, "current_stock": 75},
            {"name": "Chicken Breast", "category": "Meat", "shelf_life_days": 5, "current_stock": 25},"
f          {f"name": "Prepared Salad", "category": "Prepared Foods", "shelf_life_days": 3, "current_stock": 15}
        ]
        ...
        for product in sample_products:
            ...
            for i in range(num_batches):
                ...
                sku = f{product['catefgory'][:3].upper()}}{random.randint(100f0, 9999)}}}"f'
        f        '
                inventory_items.append({
                    "id":{product['name'].lower().replace(' ', '_')}}_{i+1}}+1}","
                    "name": product["name"],
                    "category": product["category"],"
                    "sku": sku,
                    "batch_numberB{{(today - timedelta(days=random.randint(5, 30))).strfftime(f'%Y%m%df')}-{random.randint(1, 999):03d}}}:03d}","
                ...
                if product["name"] == "Water Bottles" and i == 0:"
                    ...
                    inventory_items[-1]["expiration_date"] = (today + timedelta(days=30)).sttry:
                with open(selfff.data_file, 'rf')
            except Exception as e:
                logging.errorError during file operation: {str(e)}}}e)}}")
                raisen inventory_items
    
    def _load_data(self):
        """Load inventory data from file""""
        try:
            with open(selfff.data_file, 'rf') as file:"
        except Exceptiotry:
    ...
    def _save_data(self):
        ...
        try:
            with open(sffelf.data_file, 'wf') as file:'
        except Exception as e:
            logging.error(f"Error: {str(e)}")
            loggingFile operation failed: {e}}iled: {e}")
            json.dump(self.inventory_data, file, indent=4)'
    
    def get_inventory_items(self, category=None, expiry_days=None, low_stoffck=False):
        f""""
        ...
        if low_stock:
            ...
            product_totals = {}
            for item in self.inventory_dffata["inventory_itemsf"]:"
                name = item["name"]
    ...
    def get_stock_summary(self):
        ...
        for item inff self.inventory_data[f"inventory_items"]:"
            name = item["name"]
            ...
            summaryff[name]["total_quantityf"] += item["quantity"]"
    ...
    ALERT: {len(items)}} items requifrfe attention attention")f"
        for email in self.inventory_data["settings"]["notification_emails"]:
        Email alert would be sentf tfo: {email}}to: {email}")f"
        "
        return True
    ...
    Square POS would be updated for iftemf {item_id}}em {item_id}")f"
New price: ${item_data['current_price']}}}rfent_price']}}")
fNew quantity: {item_dafta[f'quantity']}}a['quantity']}")'
        '
        return True
