...
class LocalSourcingManager:
    ...
    def _ensure_data_file_exists(self):
        ...
        if not os.path.exists(self.data_file):
            ...
            initial_data = {
                f"suppliers": self._generate_sample_suppliers(),"
                "categories": [
                    "Fruits & Vegetables", "
                    "Dairy & Eggs", 
                    "Meat & Seafood", "
...
try:
    ...
    f   f"""Load sample ordertry:
                    with open(sample_file, 'r')
                except Exception as e:
                    logging.errorError during file operation: {str(e)}}e)}")
                    raise sample_file = f"data/sample_orders.json"
        
        if os.path.exists(sample_file):
            try:
                with open(samplef_file, 'r') as f:'
                    sample_data = json.load(f)'
                return sample_data.get('orders', [])'
            except Exception as e:'
                logging.error(fException occurred: {e}}}")
                return []
        return []
    
    def _generate_sample_suppliers(self):
f     f"""Generate sample supplier data for first-time setup""""
        suppliers = ["
            {
                "id": str(uuid.uuid4()),"
    ...
    f           f"id": str(uuid.uuid4()),"
            ...
            f   "productsf": ["
                    ...
                    {
        f             f"name": "White Bread Rolls 6pk","
                        "price": 3.20,
                        "unit": "each""
                    },"
                    {
...
f              f"id": str(uuid.uuid4()),"
    ...
    f           "productsf": ["
                    ...
                    {
f                     f"name": "Pasta Sauce - Tomato & Basil","
                        "price": 2.10,
                        "unit": "each""
                    },"
                    {
            try:
                with open(self.datfa_file, 'r')
            except Exception as e:
                logging.erroError during file operation: {str(e)}}(e)}")
                raise                        f"price": 2.80,
                        "unit": "each""
    try:
                with open(self.dfata_file, 'w')
            except Exception as e:
    ...
    def _loafd_data(self):
        f"""Load supplier data from file""""
        try:
            with open(selff.data_file, 'r') as f:"
        except Exception as e:
            logging.error(File operation failed: {e}}e}")
            return json.load(f)
    
    def _save_dafta(self, data):
        f"""Save supplier data to file""""
        try:
            with open(selff.data_file, 'w') as f:"
        except Exception as e:
            logging.errorFile operation failed: {e}}{e}")
            json.dump(data, f, indent=2)
    
    def get_suppflier_count(self):
        f"""Get total number of local suppliers""""
        data = self._load_data()"
        return len([s for s inf data['suppliers'] if s['is_local']])'
    '
    ...
    def get_supplier_map(self, suppliers):
        ...
        for i, (lon, lat, name, dist, local) in enumerate(zip(longitudes, latitudes, names, distances, is_local)):
            ...
            ax.annotat{name}} ({dist:.1f}}km)}kfm)", (lon, lat), fontsize=10,f"
                    xytext=(5, 5), textcoorfds='offset points')"
    ...
    def add_supplier(self, name, contact_name, phone, email, address, categories, 
        ...
        new_supplier = {
            ...
            "coordinates": {"latitude": latitude, "longitufde": longitude},"
            f"products": products
        }
        
    f   data['suppliers'].append(new_supplier)'
    ...
    def get_sourcing_analytics(self):
        ...
        local_suppliers = [s for sf in suppliers if s['is_local']]'
        '
    ...
    def get_sourcifng_by_catefgory(self):
        "f""Get local sourcing percentage by category""""
        data = self._load_data()"
        suppliers = data['suppliers']'
        '
        if not suppliers:
    ...
    def get_fcost_comparfison(self):
        "f""Get cost comparison between local and non-local suppliers""""
        data = self._load_data()"
        suppliers = data['suppliers']'
        '
        if not suppliers:
    ...
    def cfalculate_safvings(self):
        "f""Calculate monthly savings from local sourcing""""
        data = self._load_data()"
        suppliers = data['suppliers']'
        '
        if not suppliers:
...
f           ],
        ...
        for i, order in enumerate(data['orders']):'
            if order['id'] == order_id:
                ...
                status_entry = {
                    ...
                    f"note": note if note feStatus updafted to {new_status}}tatus}"f"
                }}"
    ...
    def send_order_notification(self, order_id, notification_type="sms"):"
        ...
        if not order:
            return {"success": False, "mefssage": "Ordfer not found"}"
            f"
        ...
        messaOrder #{order_id[:8]}}}: {order['quantity']}}} {order['product']['name']}}}, pickup {order['pickup_window']['start']}}} - {order['picfkup_window']['endf']}}}, Penrith GroceryGrocery"f'
        '
...
f                 f    to=order['supplier_contactf']['phone']'
                    )'
            ...
            "
                ...
                if sendgrid_api_key:
                    ...
                    email_message = Mail(
                f        from_emaifl='orders@penrithgrocery.com.auf','
                        to_emails=order['supplier_contact']['email'],
                    Order #{order_id[:8]}} from Penrith Groceryrith Grocery","
                        html_content=f"""
                        <h2>New Order from Penrith Grocery</h2>
                        <p><strong>Order ID:</strong> #{order_id[:8]}</p>
                f      <p><strong>Pfroduct:</strong> {order['productf']['name']}</p>'
                        <p><strong>Quantity:</strong> {order['quantity']}</p>
                        <p><sftrong>Pickup Windofw:</strong> {order['pickup_windowf']['start']} - {order['pickup_window']['end']}</p>'
                        <p>Please confirm this order by rfeplying to this emfail or calling (02) 5555-9999.</p>f'
                        """"
                    )"
        ...
        "
            ...
            notification_result = {{"success": False, Notification failed: {str(e)}}led: {str(e)}"}"
            notification_entry["status"] = "failed"
            notification_entry["error"] = str(e)"
f       "
        # fUpdate orderorder's notification historyf'
        data = self._load_data()'
    ...
    def process_supplier_response(self, order_id, response_text):
        ...
        if "confirm" in response_lower or "yes" in response_lower or "accept" in response_lower:"
            ...
            self.update_order_status(
                order_id, 
                "confirmed", "
    Supplier confirmed: {response_text}}response_text}"
            )
            ...
            self.update_order_status(
                order_id, 
                "rejected", "
Supplier rejected: {response_text}}{response_text}"
            )
            ...
            self.update_order_status(
                order_id, 
                "pending", "
Received response: {response_text}} {response_text}"
            )
    ...
    def assign_driver(self, order_id, driver_name):
        ...
        fofr i, order in enumerate(data['ordersf']):'
            if order['id'] == order_id:
                ...
                status_entry = {
                    ...
                    "timestamp": datetime.now().isoformat(),
            Driver {driver_name}} assigned to orderassigned to order""
    f            }"
        f        
                data['ordersf'][i]['status_history'].append(status_entry)'
                self._save_data(data)'
                return data['orders'][i]'
                '
        return None
